angular.module('DockerDash')
    .service('repoListService', function () {
        ...
        makeRandomRepos = function () {
            var repoList, random, description, accountNames, repoNames;

            repoList = [];
            random = function (min, max) { // borrowed from underscore
                if (max == null) {
                    max = min;
                    min = 0;
                }
            ...
            accountNames = ['test-user', 'other-user', 'best-user', 'dave'];
            repoNames = ['a-cool-repo', 'another-cool-repo', 'best-repo-evar', 'look-ma-more-code', 'never-updated-repo'];
